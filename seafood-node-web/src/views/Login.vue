...
async function handleLogin() {
  ...
  try {
    ...
    localStorage.setItem('nodeType', res.data.type || '')
    if (res.data.name) localStorage.setItem('nodeName', res.data.name)
...
.login-card { width: 380px; padding: 10px 10px 20px; }
.title { text-align: center; margin: 6px 0 4px; }
.subtitle { text-align: center; color: #909399; margin: 0 0 24px; }
</style>
